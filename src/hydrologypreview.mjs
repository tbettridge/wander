...
// connected rivers, migration and the wider visual corpus pass their gates.
...
export async function prepareWaterPreview(seed, search = '') {
  if (new URLSearchParams(search).get('waterPreview') !== 'basins') return null;
  return new Promise((resolve, reject) => {
    const worker = new Worker(new URL('./hydrologyworker.js?v=hydrology3', import.meta.url), { type: 'module' });
    worker.onmessage = ({ data }) => {
      worker.terminate();
      if (data.type === 'basins-planned') resolve({ waterPlans: [data.plan], crossingManifests: [data.manifest] });
    ...
    worker.postMessage({ type: 'plan-basins', id: 1, seed, regionX: 0, regionZ: 0 });
  });
}

export function waterPreviewSpawn(world) {
